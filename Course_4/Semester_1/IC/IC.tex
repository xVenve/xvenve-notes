...
\begin{itemize}
	\item Imposibilidad de utilización directa de las ontologías previamente desarrolladas, almacenan el conocimiento junto, pero para hacer razonamientos hay que extraerlo (con un parser) y adaptarlo.
...
\end{lstlisting}
	...
	\item \textbf{Contar el n.º de instancias de un tipo particular.}
...
\begin{enumerate}
	\item Por defecto: longitud del plan.
...
\subsubsection{Acciones durativas}
...
\begin{itemize}
	\item \textbf{valor numérico:} :duration (= ?duration 5)
	\item \textbf{valor calculado:} :duration (= ?duration (+ ?x ?y))
	\item \textbf{intervalo:} :duration (and (> ?duration 0) (< ?duration 10))
	
	(necesita requirement :duration-inequalities)
	\item \textbf{sin valor} (dura hasta que una precondición se haga falsa)
\end{itemize}
...
\begin{itemize}
	\item \textbf{Derived predicates} (axiomas de PDDL 1)
	
	Requirement: :derived-predicates

	\item \textbf{Timed initial literals}
	
	Requierement: :timed-initial-literals
...
Introduce: 
\begin{itemize}
	\item \textbf{State trajectory constraints:}
	
	Necesita requirement: :constraints 
	
	Metas intermedias 
	\item \textbf{Goal preferences:}
...
\subsubsection{State Trajectory Constraints}
...
\begin{itemize}
	\item always $\phi$: $\phi$ tiene que ser cierto en todos los estados
	\item sometime $\phi$: $\phi$ tiene que ser cierto en algún momento
	\item at-most-once $\phi$: $\phi$ tiene que ser cierto una sola vez
	\item at end $\phi$: $\phi$ tiene que ser cierto al final
	\item within t $\phi$: $\phi$ tiene que ser cierto durante t instantes de tiempo
	\item sometime-before $\phi$ $\phi_2$: $\phi$ tiene que ser cierto antes que $\phi_2$
	\item sometime-after $\phi$ $\phi_2$: $\phi$ tiene que ser cierto después que $\phi_2$
	\item always-within t $\phi$ $\phi_2$: cuando $\phi$ sea cierto, $\phi_2$ debe ser cierto antes de t instantes de tiempo
	\item hold-during $t_1$ $t_2$ $\phi$: $\phi$ tiene que ser cierto en el intervalo de tiempo entre $t_1$ y $t_2$
	\item hold-after t $\phi$: $\phi$ tiene que ser cierto después de t
\end{itemize}

\subsubsection{Preferencias}
Se pueden definir en:
\begin{itemize}
	\item Fichero de dominio: (aplicable a todos los problemas)
	
	En la parte de :constraints
	
	En las precondiciones de los operadores
	\item Fichero de problema
	
	En la parte de :constraints
	
	En la parte de :goals
\end{itemize}
Sintaxis: (preference <nombre> $\phi$)
\begin{itemize}
	\item <nombre> es opcional
	\item Varias preferencias pueden tener el mismo nombre
\end{itemize}
...
Dirección: 
\begin{itemize}
	\item Encadenamiento hacia delante (forward chaining) 
	...
	\item Encadenamiento hacia detrás (backward-chaining) 
...
\subsection{Modelo de Planificación STRIPS}
Suposiciones:
\begin{itemize}
	\item Acciones
	\begin{itemize}
		\item Deterministas
		\item Instantáneas
		\item Recursos discretos
		\item Coste unitario
	\end{itemize}
	\item Entorno
	\begin{itemize}
		\item Observabilidad total
		\item Estático
	\end{itemize}
	\item Objetivo
	\begin{itemize}
		\item Satisfacción total
		\item Métrica longitud del plan 
	\end{itemize}
\end{itemize}
...
\subsection{Modelo de Planificación con Costes}
Suposiciones:
\begin{itemize}
	\item Acciones
	\begin{itemize}
	...
	\begin{itemize}
		\item Satisfacción total
		\item Métrica \textbf{coste del plan}
	\end{itemize}
\end{itemize}
...
\subsection{Modelo de Planificación Temporal}
Suposiciones:
\begin{itemize}
	\item Acciones
	\begin{itemize}
		\item Deterministas
		\item \textbf{Durativas y concurrentes}
		\item Recursos discretos
		\item Coste unitario
	\end{itemize}
	\item Entorno
	...
	\begin{itemize}
		\item Satisfacción total
		\item Métrica \textbf{duración del plan}
	\end{itemize}
\end{itemize}
...
\subsection{Modelo de Planificación Con Recursos}
Suposiciones:
\begin{itemize}
	\item Acciones
	\begin{itemize}
...
\subsection{Modelo de Planificación con Incertidumbre}
Suposiciones:
\begin{itemize}
	\item Acciones
	\begin{itemize}
		\item Deterministas / \textbf{No deterministas}
		\item instantáneas
		\item Recursos discretos
		\item Coste unitario
	\end{itemize}
	\item Entorno
	\begin{itemize}
		\item Observabilidad total / \textbf{Observabilidad parcial / Observabilidad nula} (P. probabilidad / P. contingente / P. conformante)
		\item Estático
	\end{itemize}
	\item Objetivo
	\begin{itemize}
...
Las acciones ya no tienen lugar siempre, sin que tiene una probabilidad de fallar o no tener lugar.
...
\begin{itemize}
	\item Conocimiento completo del dominio (estado y acciones) 
	\item No hay retroalimentación (feedback) al ejecutar planes 
\end{itemize}

Planificación probabilística: 
\begin{itemize}
	\item Estado completamente observable 
	\item Acciones no deterministas: su ejecución puede producir distintos estados siguiendo una distribución de probabilidad 
	\item Necesaria retroalimentación 
\end{itemize}
