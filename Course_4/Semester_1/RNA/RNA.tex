...
En \textbf{Regresión} se utiliza el \textbf{error cuadrático medio}: $E=\frac 1 N \sum^N_{i=1} (y_i-d_i)^2$
...
\section{Con función de activación}

$$s^P_k= \sum_j w^{P-1}_{jk}y^{P-1}_j+\theta_k^P \quad y^P_k=f(s^P_k)$$

Necesitamos para cada célula un valor delta (menos para la entrada). Se deriva en función de la entrada, ya que es lo que cambia el error.
...
Para la salida (o): Siendo el error $E=\frac 1 2 \sum (y^o_k-d^o_k)^2$
$$\delta^o_k=-\frac{\partial E}{\partial y^o_k}f'(s^o_k)=-(y^o_k-d^o_k)f'(s^P_k)=(d^o_k-y^o_k)f'(s^o_k)$$

Para la oculta(h):
$$\delta^h_k=-\frac{\partial E}{\partial y^h_k}f'(s^h_k)=COMPLETAR$$
