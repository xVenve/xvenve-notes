...
Cuando Kohonen creo su modelo vio que podía sé aplicable en casos de aprendizaje supervisado, como clasificación, no solo para clustering (no supervisado). Emplearlo conociendo las salidas nos permite dividir las regiones de manera más fácil.
...
\chapter{Redes neuronales convolucionales - Deep Learning}
...
\textbf{Compresión}: Para todo vector $\vec{x}=\{x_1, x_2,\cdot, x_m\}$, obtener otro $\vec{z}=\{z_1, z_2,\cdot, z_n\}$ con $n<<m$, de forma que $\vec{x}$ pueda ser construido a partir de $\vec{z}$.

\subsection{Formulación}
La función de compresión y descompresión tendrá la forma: $\vec{z}=W_1\vec{x}+b_1$, $\vec{x}=W_2\vec{z}+b_2$.
