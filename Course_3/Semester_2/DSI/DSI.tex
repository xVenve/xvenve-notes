...
\textbf{En general, esta es una pregunta correcta en una entrevista: \enquote{¿Qué problemas tienes con este producto?}}
\begin{itemize}
	\item Falso
\end{itemize}

\textbf{En general, esta es una pregunta correcta en una entrevista: \enquote{¿crees que el producto debería ser más pequeño y barato?}}
\begin{itemize}
	\item Falso
\end{itemize}

\textbf{Marca todas las correctas. Un \enquote{Contextual Inquiry}\ldots{}}
\begin{itemize}
...
%Straight Lines [id:da13471245958847788] 
\draw    (150,40) -- (149,200) ;
%Straight Lines [id:da06234307165339659] 
\draw    (390,200) -- (149,200) ;

% Text Node
\draw (114,65) node [anchor=north west][inner sep=0.75pt]   [align=left] {yes};
% Text Node
\draw (120,151) node [anchor=north west][inner sep=0.75pt]   [align=left] {no};
% Text Node
\draw (32,102) node [anchor=north west][inner sep=0.75pt]   [align=left] {\begin{minipage}[lt]{53.75pt}\setlength\topsep{0pt}
\begin{center}
...
\end{minipage}};
% Text Node
\draw (231,241) node [anchor=north west][inner sep=0.75pt]   [align=left] {Affordance};
% Text Node
\draw (200,211) node [anchor=north west][inner sep=0.75pt]   [align=left] {no};
% Text Node
\draw (321,211) node [anchor=north west][inner sep=0.75pt]   [align=left] {yes};
% Text Node
\draw    (330.5, 160) circle [x radius= 55.86, y radius= 29.7]   ;
\draw (292,141) node [anchor=north west][inner sep=0.75pt]   [align=left] {\begin{minipage}[lt]{53.75pt}\setlength\topsep{0pt}
\begin{center}
...
\end{minipage}};
% Text Node
\draw    (330.5, 75.78) circle [x radius= 53.74, y radius= 29.7]   ;
\draw (293.5,56.78) node [anchor=north west][inner sep=0.75pt]   [align=left] {\begin{minipage}[lt]{51.87pt}\setlength\topsep{0pt}
\begin{center}
Hidden\\Affordance
\end{center}
...
\end{minipage}};
% Text Node
\draw    (212, 160) circle [x radius= 53.74, y radius= 29.7]   ;
\draw (175,141) node [anchor=north west][inner sep=0.75pt]   [align=left] {\begin{minipage}[lt]{51.87pt}\setlength\topsep{0pt}
\begin{center}
False\\Affordance
\end{center}
...
\end{minipage}};
% Text Node
\draw    (212, 75.78) circle [x radius= 47.38, y radius= 29.7]   ;
\draw (179.5,56.78) node [anchor=north west][inner sep=0.75pt]   [align=left] {\begin{minipage}[lt]{45.25pt}\setlength\topsep{0pt}
\begin{center}
Correct\\Rejection
\end{center}
...
\textbf{State of the art (\enquote{lo último})} Ver lo que hay
...
\begin{itemize}
	\item \textbf{Pronto}: Entender el espacio de diseño, los usuarios, contexto
	      y las tareas.
	\item \textbf{Más adelante}: Ver el diseño en uso, como mejora/aporta a esas
	      tareas.
\end{itemize}
...
\subsubsection{Directas en el campo}
...
\begin{itemize}
	...
	\item \textbf{Observaciones}:
	      ...
	      \begin{itemize}
		      \item \textbf{Cualitativo}: Anotaciones, grabaciones\ldots{}
		      \item \textbf{Cuantitativo}: logs, grabaciones\ldots{}
	      \end{itemize}
\end{itemize}
...
\begin{itemize}
	\item \textbf{Gruesa}: Identificación temas, a nivel general.
	\item \textbf{Fina}: A nivel de palabra, más detallado.
\end{itemize}

\textbf{Esquema de categorización:}

\begin{itemize}
	\item \textbf{Bottom-up}: De los datos hacia lo general.
	\item \textbf{Top-down (deductivo)}: esquema establecido con anterioridad
...
\subsection{Análisis de contenido (Content Analysis)}

Se utiliza para cualquier tipo de documentos y \enquote{textos} (video,
...
\subsection{Identificación de patrones y temas}
...
\hline
\textbf{Requirement:}                                                      & Unique id & \textbf{\begin{tabular}[c]{@{}l@{}}Requierement \\ Type:\end{tabular}} &                                                                               & \textbf{Event/BUC/PUC \#:} &  \\ \hline
\textbf{Description:}                                                      & \multicolumn{5}{l|}{A one sentence statement of the intention of the requirement}                                                                                                                  \\ \hline
\textbf{Rationale:}                                                        & \multicolumn{5}{l|}{A justification of the requirement}                                                                                                                                            \\ \hline
\textbf{Originator:}                                                       & \multicolumn{5}{l|}{The person who raised this requirement}                                                                                                                                        \\ \hline
\textbf{Fit Criterion:}                                                    & \multicolumn{5}{l|}{\begin{tabular}[c]{@{}l@{}}A measurement of the requirement such that it is possible to\\ test if the solution matches the original requirement\end{tabular}}                  \\ \hline
\textbf{\begin{tabular}[c]{@{}l@{}}Customer \\ Satisfaction:\end{tabular}} & \multicolumn{2}{l|}{}                                                              & \textbf{\begin{tabular}[c]{@{}l@{}}Customer \\ Dissatisfaction:\end{tabular}} & \multicolumn{2}{l|}{}         \\ \hline
\textbf{Priority:}                                                         & \multicolumn{2}{l|}{A rating of the customer value}                                & \textbf{Conflicts:}                                                           & \multicolumn{2}{l|}{}         \\ \hline
\textbf{\begin{tabular}[c]{@{}l@{}}Supporting \\ Materials:\end{tabular}}  & \multicolumn{5}{l|}{}                                                                                                                                                                              \\ \hline
\textbf{History:}                                                          & \multicolumn{5}{l|}{}                                                                                                                                                                              \\ \hline
\end{tabular}
...
\subsection{Affordances Motivacionales (Hamari, 2014)}
\begin{itemize}
	\item Invitación o motivas al usuario a realizar determinados comportamientos.
	\item Feedback visual o auditivo (o táctil)
	\item Apoyo social, comparaciones, feedback, interacción, compartir.
	\item Progreso y tracking.
	\item Mensajes y recordatorios persuasivos.
	\item Objetivos.
	\item Recompensas, créditos, punto, logros.
	\item Displays ambientales o públicos.(embebidos en el espacio)
	\item Agentes sociales.
	\item Competición, leaderboards, ranking.
	\item Emociones y expresiones.
	\item Sugerencias, consejos.
	\item Persuasión vía video.
	\item Refuerzo positivo.
	\item Persuasión subliminal.
\end{itemize}

\subsection{De Principios a Requisitos e Implementación}
Hablan de la traducción de los distintos principios y técnicas de persuasión.

Basado en el trabajo de BJ Fogg: Tríada funcional y principios de diseño.

Faltan ejemplos de cómo se traduce en el proceso de diseño.

\subsection{Apoyo a la tarea principal}
\begin{itemize}
...
\subsection{Comunicación y Diálogo de Apoyo}
\begin{itemize}
	\item Praise
	\item Reward
	\item Reminders
...
\subsection{Credibilidad}
\begin{itemize}
	\item Trustworthiness
	\item Expertise
	\item Surface credibility
	\item Real-world feel
	\item Authority
	\item Third-party endorsements
	\item Verifiability
\end{itemize}

\subsection{Apoyo Social}
\begin{itemize}
	\item Social learning
	\item Social comparison
	\item Normative influence
	\item Social facilitation
	\item Cooperation
	\item Competition
	\item Recognition
\end{itemize}

\section{Gamificación y Playificación}
Se basa en que jugamos constantemente, a todas las edades, momentos y lugares, el aspecto lúdico es muy importante en la vida de las personas.
...
\begin{itemize}
	\item \textbf{Estudios en el lab:} Lo bueno es que revela problemas de usabilidad, lo malo es que no se captura el contexto de uso e información sobre aspectos complejos de la experiencia de usuario.
	\item \textbf{Estudios in-the-wild:} La ventaja es la validez ecológica y situada, cómo usa la gente la tecnología en situaciones reales. Lo malo es que requiere tiempo, logística…
	\item \textbf{Modelos y evaluaciones predictivas:} Más rápidas, pero se le escapan muchos aspectos.
	\item \textbf{Analytics:} La ventaja es que capturan bien el uso, lo malo es que no capturan lo que los/as usuarios/as sienten, su experiencia, el por qué se comportan como lo hacen…
\end{itemize}
