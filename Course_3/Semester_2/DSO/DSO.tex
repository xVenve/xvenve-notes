...
\subsection{Imagen}
En Linux todo es un archivo, de tal manera que las imágenes se gestionaran de esta manera.

En un gran fichero comprimido con su propio sistema de ficheros, que está construido en capas.
...
Cuanto instanciamos un contenedor tenemos la capa de contenedor (sobre todas las demás), esta capa sí que es modificable para ajustar el funcionamiento de la imagen, pero el resto de las capas de la imagen son inmutables.
...
Este método en lo que consiste es en ejecutar un contenedor y sobre este realizar las modificaciones que sean necesarias, en el momento que está como deseamos lo guardamos como una imagen dándole un nombre.
\begin{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		FROM scratch
	\end{itemize}
	\item RUN es la cláusula que no permite ejecutar distintos comandos sobre el sistema de la capa base. Con barra invertida (contraria a /) se puede hacer que siga en la siguiente línea.
	\begin{itemize}
...
Lo utilizaremos para ejecutar aplicaciones multiservicio/distribuida que tiene varios contenedores.

Es una herramienta para definir y lanzar aplicaciones distribuidas con varios contenedores con un solo comando.

Nos permite integrar todas las creaciones de contendores dentro de un único fichero llamado docker-compose.yaml .
