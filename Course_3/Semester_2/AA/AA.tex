...
\(p\oplus+p\ominus=0.5\) Entropía máxima
...
Son algoritmos de búsqueda local estocástica, que las acciones tienen una probabilidad de ocurrir, que son buenos para la optimización.

Hacen búsqueda en el espacio de soluciones, pero cuando es fácil encontrar una solución, pero no la óptima. Funcionan por generaciones, en la que cada generación tiene una población, que está compuesta por p individuos, cada uno es una solución al problema.
...
\begin{itemize}
  ...
  \item Las operaciones para combinar (pequeños cambios) soluciones y generar soluciones nuevas.
  \item La función de adecuación (fitness) que indica como de buena es una población, cuanto más es mejor. Transforma el genotipo (cadena de bits) en el fenotipo (significado de la cadena bits).
\end{itemize}
...
\end{figure}
\begin{enumerate}
  \item Se selecciona, individuos de la población de soluciones. Se pueden elegir por Ruleta (por número aleatorio) o por Torneo (se coge un par de individuos y se escoge el mejor, dejando al otro fuera).
...
Los ejemplos no son de la forma atributo-valor, sino que se representan con cualquier estructura de datos, como pueden ser imágenes, documentos, planes, etc. Consigue adaptar los datos de otros problemas al actual, viendo cómo se han resuelto problemas previos.
