...
Practicas semanales por parejas.

Hay clases en el cronograma de más, algunas se usarán para recuperar clases.

Las practicas las haremos en Linux, se puede utilizar máquina virtual, guernika y WSL2, lo importante es tener un terminal.
...
\textbf{Análisis}: Parte que solo depende de la estructura del lenguaje, no de la arquitectura de la maquina.
\begin{itemize}
  ...
  \item \textbf{Analizador sintáctico:} Agrupa los componentes léxico en frases
...
Es una alternativa más sencilla, el grafo de dependencias es más completo. Se utiliza cuando no es una Gramatica de atributos por la izquierda mediante esquema de traducción con atributos sintetizados (siempre).

Se usa para los casos en los que la gramática no se puede evaluar durante el análisi sintáctico.
...
\textbf{Orden estático:} Analizando la gramática y las dependencias se establece el orden a priori.
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \end{figure}
      \item \textbf{Recorrido de varias pasadas, Recorrido combinada}, atributos heredados dependen de sintetizados.
    \end{itemize}
  \end{itemize}
