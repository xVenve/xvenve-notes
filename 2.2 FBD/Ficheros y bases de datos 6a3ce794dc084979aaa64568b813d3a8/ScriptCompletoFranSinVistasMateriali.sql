...
drop materialized view insert_proposals; 
drop materialized view insert_comments;
drop index date_index;
drop index dr_com;

create bitmap index dr_com on genres_movies(genre) tablespace tab_16k;
CREATE unique INDEX MEMBER_INDEX_PROPOSALS ON PROPOSALS(MEMBER, club,title, director);
CREATE unique INDEX NICK_INDEX_COMMENTS ON COMMENTS(NICK,club,title,director,msg_date) TABLESPACE TAB_16K;
create index date_index on users(reg_date) tablespace tab_16k;


create index nick_membership_index on membership(nick) tablespace tab_16k;

CREATE INDEX director_index_movies ON movies(director) TABLESPACE TAB_16K;
...
BEGIN
  ...
  INSERT INTO proposals(title, director, club, member, prop_date, slogan, message) 
    SELECT *
	FROM insert_proposals; 
COMMIT;
 
  INSERT INTO comments(club, nick, msg_date, title, director, subject, message, valoration) 
    SELECT *
	from insert_comments;
COMMIT;
